  ...
  const handleShareKakao = () => {
    window.Kakao.Share.sendDefault({
      ...
      content: {
        ...
        imageUrl:
          'https://images.unsplash.com/photo-1468818438311-4bab781ab9b8?w=800&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MjB8fHRyaXB8ZW58MHx8MHx8fDA%3D',
        link: {
    ...
    <Section title={'공유하기'}>
      <div className={cx('wrap-share')}>
        <button onClick={handleShareKakao}>
          <IconKakao className={cx()} />
        </button>
        <CopyToClipboard
          text={window.location.origin}
          onCopy={() => window.alert('복사가 완료되었습니다.')}
        >
          <button>
            <IconClipBoard className={cx()} />
          </button>
        </CopyToClipboard>
      </div>
    </Section>
